...
// dameda custom
#define pinHeartbeat 9            // Heartbeat Output every 100msec high at 1000msec
...
Mower::Mower(){
  ...
  batChgFactor    = 0.000;      // battery conversion factor  / 10 due to arduremote bug, can be removed after fixing (look in robot.cpp)
  ...
  chgSelection    = 2;
  chgSenseZero    = 0;        // charge current sense zero point -511
  chgFactor       = 0;         // charge current conversion factor   - Empfindlichkeit nimmt mit ca. 39/V Vcc ab -39
  chgSense        = 0;      // mV/A empfindlichkeit des Ladestromsensors in mV/A (Für ACS712 5A = 185) -185.0
  ...
  odometryTicksPerRevolution = 198;   // encoder ticks per one full resolution
...
void Mower::setup(){
 ...
 // drops
  ...
  // dameda custom
  pinMode(pinHeartbeat, OUTPUT); // 10 Hz Heartbeat for external Arduino e.g. Watchdog function.
