...
// dameda custom
#define pinHeartbeat 9             // Heartbeat Output every 100msec => 10 Hz
...
Mower::Mower(){
  ...
  batGoHomeIfBelow = 22.0;     // drive home voltage (Volt)
  batSwitchOffIfBelow = 25.8;  // switch off battery if below voltage (Volt)
  batSwitchOffIfIdle = 1;      // switch off battery if idle (minutes)
  batFactor       = 0.555;      // battery conversion factor  / 10 due to arduremote bug, can be removed after fixing (look in robot.cpp)
  batChgFactor    = 0.0;      // battery conversion factor  / 10 due to arduremote bug, can be removed after fixing (look in robot.cpp)
  ...
  chgSelection    = 2;
  chgSenseZero    = 000;        // charge current sense zero point//511
  chgFactor       = 000;         // charge current conversion factor   - Empfindlichkeit nimmt mit ca. 39/V Vcc ab//39
  chgSense        = 000.0;      // mV/A empfindlichkeit des Ladestromsensors in mV/A (Für ACS712 5A = 185)//185
  ...
  odometryTicksPerRevolution = 199;   // encoder ticks per one full resolution
  odometryTicksPerCm = 4.0;  // encoder ticks per cm
  odometryWheelBaseCm = 44;    // wheel-to-wheel distance (cm)
...
void Mower::setup(){
  ...
  //digitalWrite(pinMotorMowEnable, HIGH);  //dameda
...
void Mower::setActuator(char type, int value){
  switch (type){
    ...
    // dameda custom
    case ACT_HEARTBEAT: digitalWrite(pinHeartbeat, value); break;
  }
}
