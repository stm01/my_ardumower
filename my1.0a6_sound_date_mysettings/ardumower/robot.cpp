...
void Robot::beep(int numberOfBeeps, boolean shortbeep = false){
  for (int i=0; i < numberOfBeeps; i++){
    setActuator(ACT_BUZZER, 3000);      // dameda changed from 4200 to 3000
    if (shortbeep) delay(50); 
      else delay(200);                  //dameda set from 500 to 200
...
void Robot::setDefaultTime(){
  ...
  datetime.date.month = 8;
  datetime.date.year = 2016;
  timer[0].active = false;
  timer[0].daysOfWeek= B01111110;
  timer[0].startTime.hour = 9;
  timer[0].stopTime.hour = 11; // dameda changed to actual date & time, maybe with variables some day
}
...
// dameda Battery will be checked every 5 seconds instead of every second
void Robot::checkBattery(){
if (millis() < nextTimeCheckBattery) return;
	nextTimeCheckBattery = millis() + 30000;  
  if (batMonitor){
...
void Robot::checkHeartbeat(){
   if (millis() >= nextTimeCheckHeartbeat){
      nextTimeCheckHeartbeat = millis() + 100;
      } //10 Hz
   
     
  if (millis() <= nextTimeCheckHeartbeat -50 && millis() != nextTimeCheckHeartbeat ){
    //heartbeatSig = 1;
    setActuator(ACT_HEARTBEAT, HIGH);
    setActuator(ACT_USER_SW2, HIGH);
  }
    
      else {
        setActuator(ACT_HEARTBEAT, LOW);
        setActuator(ACT_USER_SW2, LOW);
        heartbeatSig = 0;
      }    
  }   
  ...
  switch (stateCurr) {
    case STATE_ERROR:
      ...
      if (millis() >= nextTimeErrorBeep){
        nextTimeErrorBeep = millis() + 10000; //dameda set to 10000 from 5000
        //beep(1, true);
        beepError();
      }
      //delay(100);                        
...
void Robot::beepBatLow() {
  setActuator(ACT_BUZZER, 800);
  delay(100);
  setActuator(ACT_BUZZER, 600);
  delay(100);
  setActuator(ACT_BUZZER, 400);
  delay(100);
  setActuator(ACT_BUZZER, 200);
  delay(100);
  setActuator(ACT_BUZZER, 0); 
  
  }

void Robot::beepError(){
  setActuator(ACT_BUZZER, 1000);
  delay(100);
  setActuator(ACT_BUZZER, 800);
  delay(100);
  setActuator(ACT_BUZZER, 1000);
...
void Robot::beepStartup(){
  ...
  delay(100);
  setActuator(ACT_BUZZER, 1400);
  delay(100);
  setActuator(ACT_BUZZER, 0); 
  }

void Robot::beepGoHome(){
  setActuator(ACT_BUZZER, 2000);
  delay(100);
  setActuator(ACT_BUZZER, 1800);
  delay(100);
  setActuator(ACT_BUZZER, 1600);
  delay(100);
  setActuator(ACT_BUZZER, 0); 
  }

void Robot::beepRemote(){
  setActuator(ACT_BUZZER, 900);
  delay(100);
  setActuator(ACT_BUZZER, 1100);
  delay(100);
  setActuator(ACT_BUZZER, 1300);
  delay(100);
  setActuator(ACT_BUZZER, 1500);
  delay(100);
  setActuator(ACT_BUZZER, 1700);
  delay(50);
  setActuator(ACT_BUZZER, 0); 
